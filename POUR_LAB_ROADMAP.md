...
- [x] 同 Recipe 多杯执行一致性：最近 3–6 杯按阶段比较流速误差、命中率和复现波动，定位最不稳定阶段
...
- [x] Target-aware Stage Challenge：当前杯先进入趋势预览；持续收敛但未过线时保留专项，达标后反弹/远离目标时进入恢复模式并阻止晋级，只有稳定保持目标线才允许累计连续 2 杯毕业
