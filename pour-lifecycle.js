...
export function createLifecycleClock(now = () => performance.now()) {
  ...
  let pausedTotal = 0;
  let running = false;
  function start(at = now()) { startedAt = at; pausedAt = 0; pausedTotal = 0; running = true; return snapshot(at); }
  function pause(at = now()) { if (!running || pausedAt) return snapshot(at); pausedAt = at; return snapshot(at); }
  function resume(at = now()) { if (!running || !pausedAt) return snapshot(at); pausedTotal += Math.max(0, at - pausedAt); pausedAt = 0; return snapshot(at); }
  function elapsed(at = now()) { if (!running) return 0; const effectiveNow = pausedAt || at; return Math.max(0, effectiveNow - startedAt - pausedTotal); }
  function snapshot(at = now()) { return { running, paused: !!pausedAt, startedAt, pausedAt, pausedTotal, elapsed: elapsed(at) }; }
  function reset() { startedAt = 0; pausedAt = 0; pausedTotal = 0; running = false; return snapshot(0); }
  return { start, pause, resume, elapsed, snapshot, reset };
}

export function createPageLifecycleController({ clock, onSuspend = () => {}, onResume = () => {}, now = () => performance.now(), breakStorage = globalThis.sessionStorage } = {}) {
  let suspended = false;
  let suspendedAt = 0;
  ...
  function suspend(reason = 'hidden', at = now()) {
    ...
    suspendReason = reason; suspended = true; suspendedAt = at;
    clock?.pause?.(at);
  ...
  function resume(reason = 'visible', at = now()) {
    ...
    suspended = false; clock?.resume?.(at);
    if (suspendedElapsed !== null) recordLifecycleBreak({ t: suspendedElapsed, hiddenMs, reason: suspendReason, recordedAt: Date.now() }, breakStorage);
...
const bootGhostReference = typeof globalThis.localStorage !== 'undefined' ? prepareGhostReferenceForBoot(globalThis.localStorage) : { prepared: false, cleanup: () => false };
...
if (typeof document !== 'undefined') {
  queueMicrotask(() => {
    ...
    installReplayPlayback(document);
