export function createReplayPlaybackController({
  ...
  raf=callback=>requestAnimationFrame(callback),caf=id=>cancelAnimationFrame(id),onState=()=>{},
}={}){
  let playing=false,speed=1,lastAt=null,frameId=0;
  ...
  function tick(at){
    frameId=0;if(!playing)return;if(lastAt===null)lastAt=at;
    const current=snapshot().fraction,dt=Math.max(0,Math.min(100,at-lastAt));lastAt=at;
    const durationMs=Math.max(1,Number(globalThis.__pourReplayDurationMs)||1);
    const next=Math.min(1,current+(dt*speed)/durationMs);setFraction(next);
    if(next>=1){playing=false;lastAt=null;emit();return}
    frameId=raf(tick);
  }
  function play(){if(snapshot().fraction>=1)setFraction(0);if(playing)return emit();playing=true;lastAt=null;emit();frameId=raf(tick);return snapshot()}
  function pause(){if(!playing)return emit();playing=false;lastAt=null;if(frameId){caf(frameId);frameId=0}return emit()}
  ...
  function seek(fraction){setFraction(Math.max(0,Math.min(1,Number(fraction)||0)));lastAt=null;return emit()}
...
export function installReplayPlayback(doc=globalThis.document){
  ...
  const controller=createReplayPlaybackController({
    getFraction:readFraction,setFraction:writeFraction,
